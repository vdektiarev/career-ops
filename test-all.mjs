...
const generatePdfCli = generatePdfScript.slice(
  generatePdfScript.indexOf('async function generatePDF()'),
  generatePdfScript.indexOf('/**\n * Inline url'),
);
const renderHtmlToPdfBody = generatePdfScript.slice(
  generatePdfScript.indexOf('export async function renderHtmlToPdf'),
  generatePdfScript.indexOf('const isMain ='),
);
if (/updatePDFManifest\(reportNum, outputPath, inputPath, format\)/.test(generatePdfCli) &&
    !/updatePDFManifest\(/.test(renderHtmlToPdfBody)) {
  pass('generate-pdf records CLI manifest data where report and input values are in scope');
} else {
  fail('generate-pdf manifest update uses out-of-scope CLI values');
}
